...
anders als die Fall-Demos im Portfolio (ein Anwendungsfall, mehrere Verfahren im Vergleich) zeigt diese Demo **ein** Verfahren – **Simulated Annealing** – an einem wachsenden Beispiel: dieselbe Rundtour wie in der [hill-climbing-demo](https://sebastianhanisch-hill-climbing-demo.streamlit.app/) (ein Depot, n Kundenstopps in einem 100 × 100-km-Gebiet), dieselben Nachbarschaften (2-opt, Or-opt, Tausch), dieselbe untere Schranke.
...
**Annahmeregel als Sidebar-Schalter (2026-09-22):** Metropolis' Zufallsentscheidung ($u < e^{-\Delta/T}$) ist nur eine von vier möglichen Regeln. **Threshold Accepting**, **Great Deluge** und **Late Acceptance Hill Climbing** entscheiden **deterministisch**, ohne Zufallszahl – und schlagen bei gleichem Budget (200 Tausend) trotzdem klar Hill Climbing mit Neustarts (4.9 %): Late Acceptance **2.2 %**, Threshold Accepting **2.7 %**, Great Deluge **3.2 %**. Der Zufall bringt gegenüber den drei deterministischen Regeln noch einen kleinen, aber messbaren Vorsprung. Zwei Kalibrierungsfunde dabei: Late Acceptance kippt bei zu großem Listenparameter L **katastrophal** (L=500: 2.2 %, L=1000: 44 %, L=3000: 147 % – kein Fehler, eine echte Grenze des Ringpuffers), und Great Deluges Anfangsabstand muss die **gesamte** Startlücke zur Schranke decken (nicht nur einen Zug wie bei den anderen drei Regeln) – die Demo hebt ihn automatisch an, wenn er zu knapp gewählt ist.
...
| **Annahmeregel** | ✅ Bei 200 Tausend Vorschlägen: Metropolis **1.4 %**, Late Acceptance Hill Climbing **2.2 %** (L=500), Threshold Accepting **2.7 %** (dieselben T0/T_end wie Metropolis, deterministisch), Great Deluge **3.2 %** (Anfangsabstand 300, Endabstand 2 Einheiten) – alle vier klar vor Hill Climbing mit Neustarts (4.9 %). ⚠️ **LAHC kippt bei zu großem L**: L=500 2.2 %, L=700 15.4 %, L=1000 44.2 %, L=3000 **147 %** – der Ringpuffer "kommt nicht mehr nach". ⚠️ **Great Deluges Anfangsabstand** muss die gesamte Startlücke zur Schranke decken (bei n=60 zufällig ~250–280 Einheiten, nicht nur einen Zug); zu knapp gewählt (280) wird automatisch auf die Startlänge angehoben, bleibt aber schwächer/wechselhafter; bei viel größeren Instanzen (n=200) reicht die für n=60 kalibrierte Konstante nicht mehr (Lücke > 800 % statt 8.9 % für Metropolis bei gleichem Budget) |
...
Regler: Stopps (10–200), Anteil der Stopps in Gruppen, **Nachbarschaft** (Tausch / 2-opt / Or-opt / 2-opt + Or-opt), **Annahmeregel** (Metropolis / Threshold Accepting / Great Deluge / Late Acceptance Hill Climbing), **Abkühlplan** (geometrisch / linear / logarithmisch; bei LAHC ausgeblendet), **Anfangstemperatur/-schwelle**, **Endtemperatur/-schwelle** (beim logarithmischen Plan, bei Great Deluge/LAHC ausgeblendet, der Wert bleibt erhalten; über dem Anfang wird sie auf diesen begrenzt),
**Anfangs-/Endabstand** (nur Great Deluge), **Listenlänge L** (nur LAHC), **Budget** (10 Tausend bis 2 Millionen Vorschläge), **Temperaturstufen** (bei LAHC ausgeblendet), **Startlösung**, Seed der Instanz (+ 🎲), Seed der Kette (+ 🎲).
...
Die vier neuen Presets (Annahmeregel) streuen über Instanz-Seed 35 stärker als die Metropolis-Presets (ihre deterministischen Regeln ohne Zufall in der Annahme sind empfindlicher gegenüber der einzelnen Instanz) und sind deshalb bewusst nur über das breite Urteil-Band geprüft, nicht über feste Einzelwerte; die Mittelwerte über fünf Instanzen × drei Ketten stehen oben in der Tabelle und in der Seitenleiste.

Die übrigen Presets zeigen einzelne Instanzen und Ketten – auf dieser Instanz ist die Kette bei "Zu kalt" zufällig gut (1.0 %), obwohl das Urteil `too_cold` lautet (nur 0.04 % der Vorschläge sind angenommene Verschlechterungen); die Mittelwerte über fünf Instanzen × drei Ketten stehen in der Tabelle oben und in den Hilfetexten (zu kalt im Mittel 7.1 %).

## Modell und Verfahren

- **Instanz, Nachbarschaften, Abstieg, Schranke** (`sa_scenario.py`, `sa_tour.py`): wortgleiche Kopie aus der [hill-climbing-demo](https://sebastianhanisch-hill-climbing-demo.streamlit.app/) (per Test gegen eingefrorene Werte); neu ist nur das Bewertungsbudget `max_evaluations` im Abstieg.
- **Kette** (`sa_algorithm.py`, Python/numpy von Grund auf): je Vorschlag ein zufälliger Nachbar (2-opt: gleichverteiltes Paar von Kanten, Or-opt: Segmentlänge 1–3, Position, Ziel und Richtung gleichverteilt, Tausch, oder eine Mischung), Δ in O(1) aus wenigen Kanten, eine von vier **Annahmeregeln** (`sa_accept_rules.py`, reine Funktionen), Zufallszahlen vorab gezogen; die beste Tour wird gemerkt.
  **Ein Vorschlag = ein bewerteter Nachbar**, wie das Hill Climbing seine Bewertungen zählt. Kontrollwert (Temperatur/Schwelle/Wasserspiegel) in **Vielfachen der mittleren Kantenlänge einer guten Tour** (untere Schranke geteilt durch die Zahl der Knoten), in Stufen: geometrisch, linear, logarithmisch (Hajek). **Verlauf**, Annahmequoten je Stufe und Momentaufnahmen je Stufe für die Darstellung.
- **Vier Annahmeregeln** (`sa_accept_rules.py`, `sa_algorithm.anneal(..., rule=...)`): **Metropolis** $u < e^{-\Delta/T}$ (Zufall); **Threshold Accepting** $\Delta \le T$ (dieselbe T0/T_end-Skala wie Metropolis, deterministisch); **Great Deluge** $L(\pi') \le \text{Schranke} + T$ (absolute Tourlänge gegen einen sinkenden Wasserspiegel, eigene Anfangs-/Endabstand-Regler, Anfangsspiegel automatisch auf die Startlänge angehoben); **Late Acceptance Hill Climbing** $L(\pi') \le h_{k \bmod L}$ oder $\le L(\pi)$ (Ringpuffer vergangener Tourlängen, ein Regler $L$, kein Plan).
- **Hill Climbing mit Neustarts** (`sa_evaluation.py`): Abstiege aus zufälligen Startlösungen, bis die bewerteten Nachbarn das Budget erreichen; der erste läuft immer zu Ende, weitere mit dem Rest des Budgets. Im Hauptvergleich bewertet der Abstieg nach jedem Zug alle Nachbarn neu (keine Nachbarschaftslisten, keine Don't-Look-Bits); das Experiment "Budget" zeigt zusätzlich Neustarts mit **Kandidatenliste + Don't-Look-Bits** (`sa_dlb.py`, wörtliche Kopie aus der [hill-climbing-demo](https://sebastianhanisch-hill-climbing-demo.streamlit.app/), nur 2-opt).
...
- **Vorab-Vermutungen zu den Annahmeregeln (vor dem Bau notiert, 2026-09-22 gemessen):** (1) "LAHC braucht keinen Plan und ist deshalb robuster gegen falsche Kalibrierung als Metropolis" – **widerlegt**: LAHC hat nur einen Regler (L), aber der ist selbst empfindlich – L=500 2.2 %, L=1000 bereits **44 %**, L=3000 **147 %**. Der vermeintliche Vorteil "kein Plan" verschiebt das Kalibrierungsproblem nur auf einen anderen Regler, er löst es nicht.
  (2) "Great Deluge ist empfindlich gegen die Absenkrate" – **richtig, aber der eigentliche Fund war ein anderer**: nicht die Absenkrate (Abkühlplan) war das Problem, sondern der **Anfangsabstand** – zu knapp gewählt (280 statt 300 Einheiten bei n=60), und die Suche kann von Anfang an kaum noch etwas annehmen. Ohne eine Korrektur im Code hätte das bei größeren Instanzen (n=200, ungeprüfte Kalibrierung) zu einer Lücke **über 800 %** geführt; `sa_algorithm.anneal()` hebt den Anfangsspiegel deshalb automatisch auf die tatsächliche Startlänge an.
  (3) "Der Zufall in der Annahmeregel ist verzichtbar – die drei deterministischen Regeln sind mindestens gleichwertig" – **widerlegt**: alle drei liegen hinter Metropolis (2.2 / 2.7 / 3.2 % gegen 1.4 %), wenn auch nicht dramatisch. Der Zufall hilft der Kette messbar, aus symmetrischen Sackgassen zu entkommen, in die eine feste Schwelle/ein fester Spiegel/ein fester Ringpuffer sie führen kann.
...
- **Die drei neuen Annahmeregeln:** `sa_accept_rules.py`-Funktionen einzeln gegen Handwerte (Schwellenfälle, Ties); zusätzlich eine unabhängige **Replay-Verifikation** in `tests/test_algorithm.py` – ein `debug_trace`-Modus zeichnet je Vorschlag `(Δ, Kontrollwert, angenommen)` auf, ein zweiter, von der Hauptschleife unabhängiger Python-Nachrechner reproduziert dieselbe Länge und dieselbe Annahme/Ablehnung für alle vier Nachbarschaften; Threshold Accepting: jede Annahme erfüllt Δ ≤ T, jede Ablehnung nicht; Great Deluge: der Wasserspiegel fällt monoton, jede Entscheidung stimmt mit `Schranke + T` überein; LAHC: der Ringpuffer beginnt bei der Startlänge und läuft korrekt um (Modulo-Test bei L kleiner als das Budget). Die bestehenden 20 Metropolis-Tests bleiben unverändert grün (Regressionsschutz).
...
- **Alle Zahlen der App-Texte sind als Tests hinterlegt** (Seitenleiste, Presets, Grenzen-Tabelle, Budget-, Temperatur-, Plan-, Nachbarschafts-, Stufen-, Größen-, Streuungs- und Heatmap-Aussagen, Kandidatenlisten + Don't-Look-Bits, Annahmeregel-Vergleich einschließlich des LAHC-L-Kollapses und der Great-Deluge-Anfangsabstand-Grenze, Schranke und Optimum gegen CP-SAT; jeweils Mittel über die festen Sweep-Instanzen × Ketten; positive **und** negative Aussagen; Rechenzeiten nur als Größenordnung);
  alle 12 Presets über mehrere Instanzen und Ketten in Urteil-Bändern; AppTest-Rauchtests (Voreinstellung, jedes Preset, jeder Schritt bei 10 und 60 Stopps, Stufen-Regler, ▶️ Abspielen und ▶️ Abkühlen abspielen ohne doppelte Schlüssel, ausgeblendete Endtemperatur, jede Annahmeregel einzeln inklusive Sidebar-Gating (LAHC blendet Plan/T0/Endtemperatur/Stufen aus, Great Deluge zeigt eigene Abstands-Regler), Begrenzung, Würfel-Knöpfe, Permalink-Grenzen, Extremwerte, Experimente auf Abruf, Footer).

## Dateistruktur

| Datei | Zweck |
|---|---|
| `app.py` | Streamlit-App: Schritte, Ergebnis, 📐 Sweeps, 🔬 Experimente (Budget inkl. Kandidatenliste + DLB, Annahmeregeln im Vergleich, Temperaturfenster, Streuung, Skalierung), 🚧 Grenzen, Mathe |
| `sa_algorithm.py` | Kette, Abkühlpläne, Vorschläge (2-opt, Or-opt, Tausch) |
| `sa_accept_rules.py` | Die vier Annahmeregeln (Metropolis, Threshold Accepting, Great Deluge, Late Acceptance Hill Climbing) als reine Funktionen |
...
| `sa_evaluation.py` | Analyse, Urteil, Hill Climbing mit Neustarts, Sweeps, Temperaturfenster, Skalierung, Streuung, Kandidatenlisten + Don't-Look-Bits, Annahmeregel-Vergleich |
| `sa_presets.py`, `sa_visualization.py` | Permalink/Presets (ausgeblendete Regler je Annahmeregel), Plotly-Figuren (achsengesperrt) |
| `tests/` | Kette (Boltzmann-Verteilung, Grenzfälle), Annahmeregeln (Handwerte + Replay-Verifikation), übernommener Kern, Kandidatenlisten + Don't-Look-Bits, Szenario und Auswertung, Aussagen der App, Presets, AppTest |
