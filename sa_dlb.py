"""Wortgleiche Kopie aus der hill-climbing-demo (hc_dlb.py).

Kandidatenlisten + Don't-Look-Bits fuer 2-opt: derselbe Zug wie in sa_tour (Kopie von hc_algorithm), aber statt nach jedem Zug
...
Guete wie der volle Abstieg (~7 % ueber der Schranke) mit rund 650 statt 74 000 bewerteten Nachbarn; bei gleichem
Budget schlaegt Hill Climbing mit Neustarts damit Simulated Annealing knapp (siehe das Budget-Experiment)."""
...
def dlb_descend(D, start, cand, seed=0, max_evaluations=None):
    ...
    Kandidatenpaar (c1, c2) - dieselbe Einheit wie 'bewertete Nachbarn' im vollen Abstieg (sa_tour.descend)."""
    n = len(start)
